...
|-components                                // 公共组件目录
    |-component1
        |-Component1.js                     // 组件 js 文件, 文件首字母大写, 驼峰标识, 代码采用 ES6 风格编码.
...
|-containers                                // 容器组件目录(一个容器组件就是一个页面, 它将各种功能组件组合在一起, 其主要负责组装功能组件, 接口调用以及整个页面的状态管理).
    |-home                                  // 首页容器组件
        |-Home.js                           // 容器组件 js 文件, 文件首字母大写, 驼峰标识, 代码采用ES6风格编码.
...
## 代码校验
1. webpack 编译代码时, 会自动检测 js, css, less, scss 类型文件的代码规范, 并自动修复(仅限于支持自动修复的代码).
